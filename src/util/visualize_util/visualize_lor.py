...
def visu_lorenz_map(lorenz_map : np.ndarray, p:int, r = 28, b=8/3):
    ...
    plt.savefig("image/lorenz/p_" + str(p)+".jpeg")
...
def visu_lorenz_init_sensitivity(two_data_logi_map : np.ndarray, iniPos1:float, iniPos2:float):
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111, projection = '3d')
    ax.set_title(f"sensitivity to bifurcation param")
    ax.set_xlabel("x", size = 14)
    ax.set_ylabel("y", size = 14)
    ax.set_zlabel("z", size = 14)
    ...
    plt.legend(['initPos1(x,y,z) = {:.7f}'.format(iniPos1[0, 0, 0]), 'initPos2(x,y,z) = {:.7f}'.format(iniPos2[0, 0, 0])])
    plt.savefig("sensitivity_to_bifurcation_lorenz")